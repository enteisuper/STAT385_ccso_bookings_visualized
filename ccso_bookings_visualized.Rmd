...
# used for day in jail
ccso_bookings_days = original %>% 
    ...
    mutate(time.spent.in.jail = as.numeric(difftime(Release.Date.Time, Booking.Date.Time, units = "days"))) %>%
...
ccso_bookings_days$City[!is.na(str_extract(ccso_bookings_days$City, "^CHAM\\w*"))] = "CHAMPAIGN"
ccso_bookings_days$City[!is.na(str_extract(ccso_bookings_days$City, "^U\\w*A"))] = "URBANA"
```
...
ui <- fluidPage(
    ...
    sidebarPanel(
        ...
        h3("Race"), choices = list("White" = "White", "Hispanic" = "Hispanic", "White (Hispanic)" = "White (Hispanic)", "Black" = "Black", "Asian/Pacific Islander" = "Asian/Pacific Islander", "Native American" = "Native American", "Unknown" = "Unknown"), selected = "White"),
        
        selectInput("checkMarital", 
        h3("Marital Status"), choices = list("Married" =  "Married", "Single" = "Single", "Seperated" = "Separated", "Significant Other" = "Significant Other", "Divorced" = "Divorced", "Widowed" = "Widowed", "Unknown" = "Unknown"), selected = "Married"),
        
        selectInput("checkSex", 
        h3("Sex"), choices = list("Female" = "Female", "Male" = "Male"), selected = "Female"),
        
        selectInput("checkCity", 
...
server <- function(input, output) {
    ...
    output$Plot <- renderPlot({
        ...
        ggplot(data = dat) +
            geom_histogram(aes(x = TimeSpentInJail), color = "lightblue", fill = "lightblue") +
            theme_minimal() +
            xlab("Time Spent in Jail (Days)")
